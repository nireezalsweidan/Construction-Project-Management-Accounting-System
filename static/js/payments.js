(() => {
  "use strict";
  const E={payments:"/api/payments/payments/",allocations:"/api/payments/payment-allocations/",receipts:"/api/payments/receipts/",clients:"/api/clients/clients/",suppliers:"/api/suppliers/suppliers/",clientInvoices:"/api/invoicing/client-invoices/",supplierInvoices:"/api/invoicing/supplier-invoices/"};
  ...
  function render(){
    ...
    else body.innerHTML=state.payments.map(p=>`<tr><td><button class="payment-row-link" data-payment-detail="${p.id}"><strong>${esc(p.payment_number)}</strong><span>${esc(p.reference||"View details")}</span></button></td><td><span class="${p.direction==='INCOMING'?'direction-in':'direction-out'}">${p.direction==='INCOMING'?'Received':'Paid'}</span></td><td>${esc(p.client_name||p.supplier_name||'—')}</td><td>${esc(p.payment_date)}</td><td>${esc(p.payment_method)}</td><td>${money(p.amount)}</td><td>${money(p.unallocated_amount)}</td><td><div class="payment-actions">${Number(p.unallocated_amount)>0?`<button class="quiet-button" data-allocate="${p.id}">Allocate</button>`:'<span class="status active"><i></i>Allocated</span>'}${p.direction==='INCOMING'?`<button class="quiet-button" data-receipt="${p.id}">Issue receipt</button>`:''}</div></td></tr>`).join('');
    const sum=(direction,field)=>state.payments.filter(p=>!direction||p.direction===direction).reduce((s,p)=>s+Number(p[field]),0);
    $("[data-metric=received]").textContent=money(sum('INCOMING','amount'));$("[data-metric=paid]").textContent=money(sum('OUTGOING','amount'));$("[data-metric=unallocated]").textContent=money(sum('', 'unallocated_amount'));$("[data-metric=total]").textContent=state.payments.length;
  ...
  async function loadParties(){const [clients,suppliers]=await Promise.all([all(E.clients),all(E.suppliers)]);$("[name=client]").innerHTML='<option value="">Select client</option>'+clients.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('');$("[name=supplier]").innerHTML='<option value="">Select supplier</option>'+suppliers.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('')}
  function directionFields(){const incoming=$("[name=direction]").value==='INCOMING';$("[data-client-field]").hidden=!incoming;$("[data-supplier-field]").hidden=incoming;$("[name=client]").required=incoming;$("[name=supplier]").required=!incoming;if(incoming)$("[name=supplier]").value='';else $("[name=client]").value=''}
  function showError(type,e){const n=$(`[data-${type}-error]`);n.textContent=e.message;n.hidden=false}
  async function openPayment(direction){const form=$("[data-payment-form]");form.reset();form.elements.direction.value=direction;form.elements.payment_date.value=new Date().toISOString().slice(0,10);const incoming=direction==='INCOMING';$("[data-payment-form-title]").textContent=incoming?'Receive payment from client':'Record supplier payment';$("[data-payment-form-context]").textContent=incoming?'Money received from a client. Only a client can be selected.':'Money paid to a supplier. Only a supplier can be selected.';directionFields();$("[data-payment-error]").hidden=true;$("[data-payment-dialog]").showModal();try{await loadParties()}catch(e){showError('payment',e)}}
  async function submitPayment(event){event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget));['client','supplier','reference','notes'].forEach(k=>{if(!values[k])delete values[k]});try{await api(E.payments,{method:'POST',body:JSON.stringify(values)});$("[data-payment-dialog]").close();await refresh()}catch(e){showError('payment',e)}}
  ...
  async function openPaymentDetail(id){
    ...
    try{
      ...
      const fields=[['Direction',p.direction],['Partner',p.client_name||p.supplier_name||'—'],['Date',p.payment_date],['Method',p.payment_method],['Amount',money(p.amount)],['Unallocated',money(p.unallocated_amount)],['Reference',p.reference||'—'],['Notes',p.notes||'—'],['Created',p.created_at]];
